#!/usr/bin/env python3
"""
TSA Welding Supplies (tsawelding.co.za) — session-based browse and save.

Shopify storefront; generic JSON-LD / OG extraction. Script injection skipped on
challenge/captcha paths (same pattern as Builders).
"""
...
SUPPLIER_SLUG = "tsawelding"
HOST = "tsawelding.co.za"
BASE_URL = "https://www.tsawelding.co.za/"
SOURCE_PRICE_KEY = "tsawelding_price"
TAGS = ["tsawelding", "tsa-welding", "welding"]
SESSION_FILE = Path(__file__).parent / "tsawelding_session.json"

# TSA Welding Shopify pages are slow; domcontentloaded often exceeds 60s.
PRICE_CHECK_NAV_TIMEOUT_MS = 120_000
PRICE_CHECK_CONTENT_TIMEOUT_MS = 90_000

URLS_HEADER = f"""# TSA Welding — add product page URLs here (one per line), or browse and Save in the UI.
# Store: {BASE_URL}
...
def fetch_current_pricing(url: str) -> dict | None:
    return fetch_retail_pricing_http(url, SUPPLIER_SLUG)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scrape TSA Welding Supplies (browse and save)")
