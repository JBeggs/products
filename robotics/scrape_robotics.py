#!/usr/bin/env python3
"""
Micro Robotics (robotics.org.za) — session-based browse and save.

Uses generic JSON-LD / OG extraction plus debug_capture dumps when SCRAPER_DEBUG=1
or extraction fails.
"""
...
SUPPLIER_SLUG = "robotics"
HOST = "robotics.org.za"
BASE_URL = "https://www.robotics.org.za/"
SOURCE_PRICE_KEY = "robotics_price"
TAGS = ["robotics", "micro-robotics", "electronics"]
SESSION_FILE = Path(__file__).parent / "robotics_session.json"
PERSISTENT_PROFILE_DIR = Path(__file__).parent / "robotics_chrome_profile"

URLS_HEADER = f"""# Micro Robotics — add product page URLs here (one per line), or browse and Save in the UI.
# Store: {BASE_URL}
...
def scrape_current_page(page, output_dir: Path) -> bool:
    ...
    if not default_is_product_url(url, HOST):
        if dbg:
            capture_page_debug(page, output_dir, SUPPLIER_SLUG, "skip_non_pdp", dbg, "URL heuristic did not match PDP")
        return False
...
def run_scrape_session(
    ...
    config = GenericScraperConfig(
        ...
        # Avoid script injection on Cloudflare challenge/login paths.
        skip_script_on_paths=("cdn-cgi", "challenge", "login"),
        # Use a persistent profile so Cloudflare clearance/session sticks between runs.
        use_persistent_context=True,
        persistent_user_data_dir=PERSISTENT_PROFILE_DIR,
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scrape Micro Robotics (browse and save)")
