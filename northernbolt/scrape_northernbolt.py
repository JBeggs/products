#!/usr/bin/env python3
"""
Northern Bolt (northernbolt.co.za) — session-based browse and save.
...
SUPPLIER_SLUG = "northernbolt"
HOST = "northernbolt.co.za"
BASE_URL = "https://www.northernbolt.co.za/"
SOURCE_PRICE_KEY = "northernbolt_price"
TAGS = ["northernbolt", "northern-bolt"]
SESSION_FILE = Path(__file__).parent / "northernbolt_session.json"

URLS_HEADER = f"""# Northern Bolt — add product page URLs here (one per line), or browse and Save in the UI.
# Store: {BASE_URL}
...
def scrape_current_page(page, output_dir: Path) -> bool:
    url = page.url or ""
    dbg = combined_debug(debug_flag=False)
...
def scrape_url(page, url: str, output_dir: Path, debug: bool = False) -> None:
    """URL-list mode (optional): open one URL and save if it looks like a PDP."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scrape Northern Bolt (browse and save)")
