"""
Makro crawler SQLite schema and access helpers.
Mirrors Gumtree db structure with ad_id used for Makro pid.
"""
import sqlite3
import threading
...
DB_PATH = Path(__file__).resolve().parent / "makro_crawler.db"
...
def get_listing_by_ad_id(ad_id: str) -> dict | None:
    """Get listing by ad_id (Makro pid)."""
    with get_db() as conn:
...
def upsert_listing(
    ...
    if existing:
        ...
        # Preserve existing title if incoming is None/empty
        final_title = title if (title and title.strip()) else existing.get("title")
        with get_db() as conn:
            conn.execute(
                """UPDATE listings SET
                ...
                WHERE id = ?""",
                (url, final_title, price, category, location, seller, condition, description, now, search_job_id, now, listing_id),
            )
...
def listing_matches_ignore(listing: dict, rules: list[dict]) -> bool:
    ...
    ad_id = str(listing.get("ad_id") or "").lower()
