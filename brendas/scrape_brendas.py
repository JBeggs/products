#!/usr/bin/env python3
"""
Brenda's Preserves (brendas.co.za) — session-based browse and save.
...
SUPPLIER_SLUG = "brendas"
HOST = "brendas.co.za"
BASE_URL = "https://www.brendas.co.za/"
SOURCE_PRICE_KEY = "brendas_price"
TAGS = ["brendas", "brendas-preserves"]
SESSION_FILE = Path(__file__).parent / "brendas_session.json"

URLS_HEADER = f"""# Brenda's Preserves — product URLs (one per line), or browse and Save in the UI.
# Store: {BASE_URL}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scrape Brenda's Preserves (browse and save)")
