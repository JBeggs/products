"""
Junk Mail crawler SQLite schema and access helpers.
"""
...
DB_PATH = Path(__file__).resolve().parent / "junkmail_crawler.db"
...
def ensure_default_config() -> None:
    """Seed default scenarios, location preferences, and crawl limits if missing."""

    for scenario in get_default_scenarios():
        upsert_scenario_config(scenario, update_existing=False)
    sync_default_search_urls()
    if get_location_preferences() is None:
        save_location_preferences(get_default_location_preferences())
    ensure_default_crawler_filters()


def sync_default_search_urls() -> None:
    """Rebuild stored search URLs from each row's query/category_path/prices."""

    from junkmail_crawler.scenario_edit import normalize_junkmail_searches

    for cfg in list_scenario_configs():
        slug = (cfg.get("slug") or "").strip()
        searches = list(cfg.get("searches") or [])
        if not slug or not searches:
            continue
        try:
            rebuilt = normalize_junkmail_searches(searches)
        except ValueError:
            continue
        if any(
            (before.get("url") != after.get("url") for before, after in zip(searches, rebuilt))
            or len(searches) != len(rebuilt)
        ):
            save_scenario_config(slug, {"searches": rebuilt})


def ensure_default_crawler_filters() -> None:
    """Seed Junk Mail crawl limits."""
    from .crawler_limits import (
        DEFAULT_MAX_PAGES_PER_SEARCH,
        DEFAULT_MAX_STORED_PER_SEARCH,
        DEFAULT_STORE_ONLY_SCENARIO_MATCHES,
    )

    defaults = {
        "max_pages_per_search": str(DEFAULT_MAX_PAGES_PER_SEARCH),
        "max_stored_per_search": str(DEFAULT_MAX_STORED_PER_SEARCH),
        "store_only_scenario_matches": "true" if DEFAULT_STORE_ONLY_SCENARIO_MATCHES else "false",
    }
    existing = get_crawler_filters()
    for key, value in defaults.items():
        if key not in existing:
            set_crawler_filter(key, value)
    # Align with Gumtree: store all listings; scenario tabs control visibility.
    if str(existing.get("store_only_scenario_matches", "")).strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
    ):
        set_crawler_filter("store_only_scenario_matches", "false")
...
def upsert_listing(
    ...
    url = normalize_junkmail_listing_url(url) or (url or "").strip()
...
def save_scenario_config(slug: str, updates: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if "slug" not in merged:
        merged["slug"] = slug
    if merged.get("searches"):
        merged["searches"] = normalize_junkmail_searches(merged["searches"])
...
def patch_listing(
    ...
    *,
    ...
    """Update listing notes, ignored flag, and/or URL. Returns updated listing or None."""
    ...
    if url is not None:
        ...
        normalized = normalize_junkmail_listing_url(url) or url.strip()
        updates.append("url = ?")
