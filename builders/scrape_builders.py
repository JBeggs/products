#!/usr/bin/env python3
"""
Builders (builders.co.za) — session-based browse and save.

Large catalogue site; extraction uses shared JSON-LD / OG / price heuristics.
...
SUPPLIER_SLUG = "builders"
HOST = "builders.co.za"
BASE_URL = "https://www.builders.co.za/"
SOURCE_PRICE_KEY = "builders_price"
TAGS = ["builders", "builders-warehouse"]
SESSION_FILE = Path(__file__).parent / "builders_session.json"

URLS_HEADER = f"""# Builders — product URLs (one per line), or browse and Save in the UI.
# Store: {BASE_URL}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scrape Builders (browse and save)")
